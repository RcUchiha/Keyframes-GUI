...
class KeyframeGeneratorApp(QMainWindow):
    ...
    def check_dependencies(self):
        command = ["python", "keyframes.py", "--check"]
        ...
        try:
            ...
            for line in output.split('\n'):
                if "no encontrado en el PATH en Windows" in line:
                    missing_program = line.split(" ")[0] # Tomamos la primera palabra de la línea como el programa faltante
                    missing_programs.append(missing_program)
                elif "no está instalado" in line:
                    if "Python" in line:
                        missing_module = line.split(" ")[0] # Tomamos la primera palabra de la línea como el módulo faltante
                        missing_python_modules.append(missing_module)
                    elif "VapourSynth" in line:
                        missing_module = line.split(" ")[0]
                        missing_vs_modules.append(missing_module)
            ...
            if missing_python_modules:
                error_message += f"Los siguientes módulos de Python no están disponibles: {', '.join(missing_python_modules)}\n"
            
            if missing_vs_modules:
                error_message += f"Los siguientes módulos de VapourSynth no están disponibles: {', '.join(missing_vs_modules)}\n"
            ...
            QMessageBox.critical(self, "Error", f"Hubo un error al ejecutar el comando: {e}")
